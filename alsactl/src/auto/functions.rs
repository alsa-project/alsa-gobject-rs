...
pub fn card_id_list() -> Result<Vec<u32>, glib::Error> {
    unsafe {
        ...
        let _ = ffi::alsactl_get_card_id_list(&mut entries, entry_count.as_mut_ptr(), &mut error);
        if error.is_null() {
...
pub fn card_sysname(card_id: u32) -> Result<glib::GString, glib::Error> {
    unsafe {
        ...
        let mut error = ptr::null_mut();
        let _ = ffi::alsactl_get_card_sysname(card_id, &mut sysname, &mut error);
        if error.is_null() {
...
pub fn control_devnode(card_id: u32) -> Result<glib::GString, glib::Error> {
    unsafe {
        ...
        let mut error = ptr::null_mut();
        let _ = ffi::alsactl_get_control_devnode(card_id, &mut devnode, &mut error);
        if error.is_null() {
...
pub fn control_sysname(card_id: u32) -> Result<glib::GString, glib::Error> {
    unsafe {
        let mut sysname = ptr::null_mut();
        let mut error = ptr::null_mut();
        let _ = ffi::alsactl_get_control_sysname(card_id, &mut sysname, &mut error);
        if error.is_null() {
            Ok(from_glib_full(sysname))
        } else {
            Err(from_glib_full(error))
