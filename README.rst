...
* FFI crates (``alsactl-sys``, ``alsahwdep-sys``, ``alsarawmidi-sys``, ``alsatimer-sys``, ``alsaseq-sys``)

  * ``libc`` >= 0.2
  * ``glib-sys`` >= 0.15
  * ``gobject-sys`` >= 0.15

* API crates (``alsactl``, ``alsahwdep``, ``alsarawmidi``, ``alsatimer``, ``alsaseq``)

  * ``libc`` >= 0.2
  * ``bitflags`` >= 1.0
  * ``glib`` >= 0.15
  * FFI crate per each (``alsactl-sys``, ``alsahwdep-sys``, ``alsarawmidi-sys``, ``alsatimer-sys``, ``alsaseq-sys``)

  * ``alsaseq`` specifically has dependency to ``alsatimer``.
