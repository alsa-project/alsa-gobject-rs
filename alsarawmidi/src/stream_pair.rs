...
impl<O: IsA<StreamPair>> StreamPairExtManual for O {
    fn protocol_version(&self) -> Result<&[u16; 3], glib::Error> {
        unsafe {
            ...
            let is_ok = ffi::alsarawmidi_stream_pair_get_protocol_version(
    ...
    fn substream_status<P: IsA<SubstreamStatus>>(
        ...
        unsafe {
            ...
            let is_ok = ffi::alsarawmidi_stream_pair_get_substream_status(
                self.as_ref().to_glib_none().0,
                direction.into_glib(),
    ...
    fn read_from_substream(&self, buf: &mut [u8]) -> Result<usize, glib::Error> {
        unsafe {
            ...
            let is_ok = ffi::alsarawmidi_stream_pair_read_from_substream(
                self.as_ref().to_glib_none().0,
                &mut buf.as_mut_ptr(),
                &mut len,
                &mut error,
